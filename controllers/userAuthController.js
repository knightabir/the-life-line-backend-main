...
exports.register = async (req, res) => {
  try {
    const { name, avatar, email, employeeId, designation, bloodgroup, shopNo, phone, password } = req.body;
    const existingUser = await User.findOne({ email });
    if (existingUser)
      return res.status(400).json({ message: 'User already exists' });

    
    const user = await User.create({ name, avatar, email, bloodgroup, employeeId, designation, shopNo, phone, password  });
    const token = generateToken(user);
    user.refreshedToken = token;
    await user.save();

    res.status(201).json({ user: { name: user.name, phone: user.phone, bloodgroup:user.bloodgroup, employeeId: user.employeeId, email: user.email }, token });
  } catch (err) {
